...
export async function poster(url, d) {
    try {
        const req = () => apiAxios.post(url, d, { withCredentials: true })
        const { data } = await handleRequest(req)
        if (data.error) {
...
export async function putter(url, d) {
    try {
        const req = () => apiAxios.put(url, d, { withCredentials: true })
        const { data } = await handleRequest(req)
        if (data.error) {
            return [data.error, null]
        }
    ...
    try {
        const req = () => apiAxios.delete(url, { withCredentials: true })
        const { data } = await handleRequest(req)
        if (data.error) {
            return [data.error, null]
        }
